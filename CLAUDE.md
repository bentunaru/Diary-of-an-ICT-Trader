...
- `ICT_Dashboard.html` — the main deliverable. A single-file, data-driven dashboard (~130KB, design V2 « ICT Dashboard modernisation »). All data lives in a `const DATA = {}` object at the top of the inline `<script>`; a `const DEMO` block right after it renders sample data **only while `DATA.trades` is empty** (badge DÉMO, cliquable pour désactiver — persisté en localStorage). Rendering is vanilla JS regenerated from `DATA`; 5 tabs (`apercu`, `journal`, `preparation`, `playbook`, `compte`) driven by `data-go` buttons + hash. Thème clair/sombre (`data-theme` sur `<html>`), courbe d'equity en SVG pur — plus de Chart.js ; seule dépendance réseau : Google Fonts (IBM Plex, fallback système). ⚠ Dans `DEMO.challenge`, `status` est volontairement placé AVANT `propFirmId` pour que `compute_daily_stop()` (update_dashboard.py) ne prenne jamais la démo pour un challenge actif — préserver cet ordre.
